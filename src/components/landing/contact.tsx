...
export function Contact() {
  ...
  return (
    <section id="contact" className="py-20 sm:py-28 bg-card text-card-foreground">
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollAnimationWrapper>
          <div className="text-center">
            <h2 className="font-serif text-3xl font-bold tracking-tight sm:text-4xl">Let&apos;s Connect</h2>
        ...
        <div className="mt-16 grid max-w-4xl mx-auto gap-12 md:grid-cols-2">
          <ScrollAnimationWrapper delay={100}>
            <div className="flex flex-col gap-6">
              <h3 className="font-serif text-2xl font-semibold">Contact Information</h3>
              <ul className="space-y-4">
                {contactDetails.map((detail, index) => (
                  <li key={index} className="flex items-center gap-4">
                    <detail.icon className="h-5 w-5 text-accent" />
                    <Link href={detail.href} className="text-muted-foreground hover:text-card-foreground transition-colors break-all">
                      {detail.text}
                    </Link>
                  </li>
                ))}
